...
# ----------------------------------------------------------
# PLOTTING FUNCTION (black text + auto arrows)
# ----------------------------------------------------------
plot_hcc_final <- function(data, sex_label) {
  ...
  ggplot() +
    ...
    # Label with optional arrows (only when overlapping)
    geom_text_repel(
      data = label_points,
      ...
      force = 2,         # gentle repelling so only crowded ones move
      max.overlaps = 50, # allows most labels to stay inside
      nudge_x = 0,
      nudge_y = 0
    ) +
    scale_fill_gradient(
