...
plot_hcc_final <- function(data, sex_label) {
  ...
  # Define which HCCs get arrows
  arrow_hccs <- c(6, 14.1, 14.3, 22.1)
  ...
  # Extend arrows outward for the specified HCCs
  centroids <- centroids %>%
    mutate(
      xend = ifelse(com %in% arrow_hccs, X + 0.3, X),
      yend = ifelse(com %in% arrow_hccs, Y + 0.3, Y),
      label_x = ifelse(com %in% arrow_hccs, X + 0.35, X),
      label_y = ifelse(com %in% arrow_hccs, Y + 0.35, Y)
    )
  ...
  ggplot() +
    ...
    # Add arrows only for selected HCCs
    geom_segment(
