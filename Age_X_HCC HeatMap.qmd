...
# Make zone outlines continuous (fixed geometry)
zones_union <- st_union(st_make_valid(zones))

# ----------------------------------------------------------
# PLOTTING FUNCTION (with arrows)
# ----------------------------------------------------------
plot_hcc_with_arrows <- function(data, sex_label) {
  ...
  ggplot() +
    ...
    geom_text_repel(
      ...
      color = ifelse(label_points$Chronic_Pct < 25, "black", "white"),
      size = 4.2,
      fontface = "bold",
      segment.color = "black",
      segment.size = 0.4,
...
# CREATE MAPS
# ----------------------------------------------------------
map_men   <- plot_hcc_with_arrows(hcc_map, "Men")
map_women <- plot_hcc_with_arrows(hcc_map, "Women")
map_total <- plot_hcc_with_arrows(hcc_map, "Total")
