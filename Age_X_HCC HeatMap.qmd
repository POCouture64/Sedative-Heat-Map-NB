...
plot_hcc_final <- function(data, sex_label) {
  ...
  # Define which HCCs get arrows pointing outside (removed 5, 7, 11, 14.1, 14.2)
  ...
  centroids <- centroids %>%
    mutate(
      # Northeast cluster - only 6 and 8 get arrows
      xend = case_when(
        com == 6 ~ X + 0.75,
        com == 8 ~ X + 0.85,
      ...
      label_y = case_when(
        ...
        com == 22.1 ~ Y - 0.77,
        com == 11 ~ Y,
        com == 14.1 ~ Y - 0.05,  # slightly below centroid
        TRUE ~ Y
      )
    )
