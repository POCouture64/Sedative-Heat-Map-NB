...
# --- Arrows only for these HCCs ---
arrow_labels <- c(6, 14.1, 14.3, 22.1)

# --- Plot function (smaller text, repel for all) ---
plot_hcc_final <- function(data, sex_label) {
  ...
  ggplot() +
    ...
    # Regular labels with repel (slight nudging to avoid overlap)
    geom_text_repel(
      data = text_data,
    ...
    geom_text_repel(
      ...
      nudge_x = 1.5,
      nudge_y = 1.5,
      arrow = arrow(length = unit(0.35, "inches"), type = "closed")
    ) +
    
    # Custom HCC6 curve + label
    geom_curve(
