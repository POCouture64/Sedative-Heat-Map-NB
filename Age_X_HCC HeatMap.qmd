...
# ---- Save the map ----
save_dir <- here::here("figures")
if (!dir.exists(save_dir)) dir.create(save_dir)

ggsave(
  filename = here::here("figures", "canada_sleeping_pills_heatmap_FINISH.png"),
  plot = canada_plot,
  width = 14,
  height = 10,
  dpi = 300
...
plot_hr_map <- function(data, fill_column, title_text, file_name) {
  ...
  gg <- ggplot(data) +
    ...
    scale_fill_gradient(low = "#e6d3fa", high = "#3f007d", na.value = "grey90") +
  ...
  # Save as PNG
  ragg::agg_png(file_name, width = 4000, height = 3200, res = 700)
  print(gg)
  dev.off()
...
# --- Example usage ---
map_any_use <- plot_hr_map(hr_map_data, "Any_Use_Perc", "Any BZRA Use by Health Region (%)", "HR_Map_Any_Use.png")
map_chronic_use <- plot_hr_map(hr_map_data, "Chronic_Use_Perc", "Chronic BZRA Use by Health Region (%)", "HR_Map_Chronic_Use.png")
```
...
map_men_any <- plot_hr_sex_map(
  hr_men_map, 
  "Any_Use_Perc", 
  "Any BZRA Use by Health Region - Men (%)", 
...
map_men_chronic <- plot_hr_sex_map(
  hr_men_map, 
  "Chronic_Use_Perc", 
  "Chronic BZRA Use by Health Region - Men (%)", 
...
map_women_any <- plot_hr_sex_map(
  hr_women_map, 
  "Any_Use_Perc", 
  "Any BZRA Use by Health Region - Women (%)", 
...
map_women_chronic <- plot_hr_sex_map(
  hr_women_map, 
  "Chronic_Use_Perc", 
  "Chronic BZRA Use by Health Region - Women (%)", 
...
for(s in sexes) {
  for(a in age_groups) {
    ...
    map_title <- paste(s, a, "Chronic BZRA Use by Health Region (%)")
