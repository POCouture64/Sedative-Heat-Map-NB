...
plot_hcc_final <- function(data, sex_label) {
  ...
  # HCCs that use arrows
  arrow_hccs <- c(6, 8, 14.3, 22.1)
  
  # Adjust positions and label locations
  centroids <- centroids %>%
    mutate(
      xend = X,
      yend = Y,
    ...
    mutate(
      ...
      label_x = case_when(
        ...
        com == 14.3 ~ X + 0.42,
        com == 22.1 ~ X + 2.3,
        com == 11 ~ X - 0.05,   # slightly more right
      ...
      label_y = case_when(
        ...
        com == 22.1 ~ Y - 0.77,
        com == 11 ~ Y + 0.10,
        com == 14.1 ~ Y - 0.25, # significantly lower
        TRUE ~ Y
      )
    )
  
  # Convert bounding box to sf polygon
  bbox_poly <- st_as_sfc(st_bbox(hcc_map))
  
  # ----------------------------------------------------------
  # PLOT
  # ----------------------------------------------------------
  ggplot() +
    # Outer border (converted properly from bbox)
    geom_sf(data = bbox_poly, fill = NA, color = "grey50", linewidth = 1) +
